...
match_tree <- function(add_poly = F, tree_to_match = list(), tree_path = "./data/Papilionidae_MCC_clean.tre" , meanphen_match, data_match, plot_match = F, genre=0){
  ...
  if (add_poly==T){
    ...
    if(is.null(data_match$tipsgenre)){
      data_match$tipsgenre <- sapply(rownames(data_match),function(x) iconv(x, from = 'latin1', to = 'ASCII//TRANSLIT'))
    data_match$tipsgenre<-gsub("_[a-z]*[A-Z]*$","",data_match$tipsgenre)
    data_match$genresp <- rownames(data_match)
    }
    
    data_match$tipsgenre<-gsub("_[a-z]*[A-Z]*$","",data_match$genresp)
